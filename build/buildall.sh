...
for host in $cpu_host_systems; do
    for device in $cpu_device_systems; do
        cd $THRUST_EXAMPLES
        scons host_backend=$host backend=$device run_examples
        cd $THRUST_TESTING
        scons host_backend=$host backend=$device
        ./tester
    done
done
...
if (( num_gpus > 0)); then
  ...
  for host in $cpu_host_systems; do
      for ((device_id=0;device_id<num_gpus;device_id++)); do
          ...
          echo cd $THRUST_EXAMPLES
          echo scons host_backend=$host backend=cuda arch=$arch run_examples
          echo cd $THRUST_TESTING
          echo scons host_backend=$host backend=cuda arch=$arch
          echo ./tester --device=$device_id
      done
  done
fi
