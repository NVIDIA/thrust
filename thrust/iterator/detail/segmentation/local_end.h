...
template<typename Iterator>
  ...
  >::type
    local_end(thrust::detail::segmented_iterator<Iterator> iter)
{
  return thrust::experimental::end(*iter.current_bucket());
}
...
template<typename UnaryFunc, typename SegmentedIterator, typename Reference, typename Value>
  typename thrust::detail::local_iterator<
    ...
    >
  >::type
    local_end(thrust::transform_iterator<UnaryFunc,SegmentedIterator,Reference,Value> iter)
{
  return thrust::make_transform_iterator(local_end(iter.base()), iter.functor());
}
