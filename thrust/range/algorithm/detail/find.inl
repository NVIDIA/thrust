...
#include <thrust/range/algorithm/find.h>
...
{
  typedef typename detail::find_result<const SinglePassRange>::type Result;
  return Result(thrust::find(begin(rng), end(rng), value), end(rng));
} // end find()
...
{
  typedef typename detail::find_result<const SinglePassRange>::type Result;
  return Result(thrust::find_if(begin(rng), end(rng), pred), end(rng));
} // end find_if()
