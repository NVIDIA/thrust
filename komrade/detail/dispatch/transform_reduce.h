...
#include <komrade/detail/host/transform_reduce.h>
#include <komrade/detail/device/cuda/reduce.h>
                                     ...
                                     komrade::detail::util::Bool2Type<true>)
{
  ...
  OutputType result = komrade::detail::device::cuda::reduce(wide_func, n_wide, init, binary_op);
  ...
  return komrade::detail::device::cuda::reduce(func, end - tail_begin, result, binary_op);
}
                                     ...
                                     komrade::detail::util::Bool2Type<false>)
{
  ...
  return komrade::detail::device::cuda::reduce(func, end - begin, init, binary_op);
}
...
{
    return komrade::detail::host::transform_reduce(begin, end, unary_op, init, binary_op);
}
...
{
    typedef typename komrade::iterator_traits<InputIterator>::value_type InputType;
    const bool use_wide_load = komrade::detail::is_pod<InputType>::value && (sizeof(InputType) == 1 || sizeof(InputType) == 2);
    return detail::transform_reduce_device(begin, end, unary_op, init, binary_op, komrade::detail::util::Bool2Type<use_wide_load>());
}
