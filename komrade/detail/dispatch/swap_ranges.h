...
template<typename ForwardIterator1,
  ...
  ForwardIterator2 swap_ranges(ForwardIterator1 first1,
                               ...
                               komrade::random_access_device_iterator_tag)
{
  typedef typename komrade::iterator_traits<ForwardIterator1>::value_type ValueType1;
  typedef typename komrade::iterator_traits<ForwardIterator2>::value_type ValueType2;
...
template<typename ForwardIterator1,
  ...
  ForwardIterator2 swap_ranges(ForwardIterator1 first1,
                               ...
                               komrade::random_access_device_iterator_tag)
{
  typedef typename komrade::iterator_traits<ForwardIterator2>::value_type DeviceType;
  typename komrade::iterator_traits<ForwardIterator1>::difference_type N = std::distance(first1, last1);

  // copy device range to temp buffer on host
  DeviceType * buffer = reinterpret_cast<DeviceType *>(malloc(N * sizeof(DeviceType)));
  komrade::copy(first2, first2 + N, buffer);

  // copy host range to device
  komrade::copy(first1, last1, first2);
                               ...
                               komrade::forward_host_iterator_tag)
{
  ...
  komrade::swap_ranges(first2, last2, first1);
  return last2;
}
