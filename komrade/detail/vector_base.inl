...
// define our own min() function rather than #include <komrade/extrema.h>
template<typename T>
...
{
  reserve(v.size());
  komrade::uninitialized_copy(v.begin(), v.end(), begin());
  mSize = v.size();
} // end vector_base::vector_base()
      ...
      ::operator=(const vector_base &v)
{
  ...
  if(num_remaining > 0)
  {
    komrade::uninitialized_copy(v.begin() + size(),
                                v.end(),
                                begin() + size());
  } // end if
  else
  {
    ...
    komrade::detail::destroy(begin() + v.size(), end());
  } // end if
  ...
  template<typename IteratorOrIntegralType>
    void vector_base<T,Alloc>
      ::init_dispatch(IteratorOrIntegralType begin,
                      ...
                      false_type)
{
  resize(komrade::distance(begin, end));

  komrade::copy(begin, end, this->begin());
} // end vector_base::init_dispatch()
         ...
         mCapacity(0)
{
  ...
  typedef komrade::detail::is_integral<InputIterator> Integer;
    ...
    ::resize(size_type new_size, value_type x)
{
  ...
  if(capacity() >= new_size)
  {
    if(new_size > size())
    {
      ...
      komrade::uninitialized_fill(begin() + size(), begin() + new_size, x);
    } // end if
    else if(new_size < size())
    {
      // destroy elements at the end of the new range
      komrade::detail::destroy(begin() + new_size, end());
    } // end else if
  ...
  if(n > capacity())
  {
    ...
    if(new_array != iterator(pointer(static_cast<T*>(0))))
    {
      ...
      if(size() > 0)
      {
        ...
        komrade::copy(begin(), end(), new_array);
    ...
    ::erase(iterator begin, iterator end)
{
  ...
  komrade::copy(end, this->end(),begin);
...
template<typename T, typename Alloc>
  void vector_base<T,Alloc>
    ::swap(vector_base &v)
{
  komrade::swap(mBegin,     v.mBegin);
  komrade::swap(mSize,      v.mSize);
  komrade::swap(mCapacity,  v.mCapacity);
  komrade::swap(mAllocator, v.mAllocator);
