...
__global__
void max_element_kernel(Iterator first, Iterator last, BinaryPredicate pred, Iterator2 result)
{
  *result = thrust::max_element(thrust::seq, first, last, pred);
}
...
void TestMaxElementDeviceSeq(const size_t n)
{
  ...
  max_element_kernel<<<1,1>>>(d_data.begin(), d_data.end(), d_result.begin());
  ...
  max_element_kernel<<<1,1>>>(d_data.begin(), d_data.end(), thrust::greater<T>(), d_result.begin());
