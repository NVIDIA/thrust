...
__global__
void equal_kernel(Iterator1 first1, Iterator1 last1, Iterator2 first2, Iterator3 result)
{
  *result = thrust::equal(thrust::seq, first1, last1, first2);
}


template<typename Iterator1, typename Iterator2, typename BinaryPredicate, typename Iterator3>
__global__
void equal_kernel(Iterator1 first1, Iterator1 last1, Iterator2 first2, BinaryPredicate pred, Iterator3 result)
{
  *result = thrust::equal(thrust::seq, first1, last1, first2, pred);
}
...
void TestEqualDeviceSeq(const size_t n)
{
  ...
  //empty ranges
  equal_kernel<<<1,1>>>(d_data1.begin(), d_data1.begin(), d_data1.begin(), d_result.begin());
  ASSERT_EQUAL(d_result[0], true);
  
  //symmetric cases
  equal_kernel<<<1,1>>>(d_data1.begin(), d_data1.end(), d_data1.begin(), d_result.begin());
  ASSERT_EQUAL(d_result[0], true);
  ...
  if(n > 0)
  {
    ...
    //different vectors
    equal_kernel<<<1,1>>>(d_data1.begin(), d_data1.end(), d_data2.begin(), d_result.begin());
    ASSERT_EQUAL(d_result[0], false);
    
    //different predicates
    equal_kernel<<<1,1>>>(d_data1.begin(), d_data1.begin() + 1, d_data2.begin(), thrust::less<T>(), d_result.begin());
    ASSERT_EQUAL(d_result[0], true);
    equal_kernel<<<1,1>>>(d_data1.begin(), d_data1.begin() + 1, d_data2.begin(), thrust::greater<T>(), d_result.begin());
    ASSERT_EQUAL(d_result[0], false);
  }
}
