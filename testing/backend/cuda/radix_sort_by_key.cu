...
__global__ void stable_radix_sort_by_key_kernel(ExecutionPolicy exec, Iterator1 keys_first, Iterator1 keys_last, Iterator2 values_first, Iterator3 is_supported)
{
...
void TestRadixSortByKeySimpleDeviceDevice()
{
  ...
  stable_radix_sort_by_key_kernel<<<1,1>>>(thrust::device, unsorted_keys.begin(), unsorted_keys.end(), unsorted_values.begin(), is_supported.begin());
...
DECLARE_UNITTEST(TestRadixSortByKeySimpleDeviceDevice);
...
template<typename T>
struct TestRadixSortByKeyDeviceDevice
{
  void operator()(const size_t n)
  {
    thrust::host_vector<T>   h_keys = unittest::random_integers<T>(n);
    ...
    thrust::host_vector<unsigned int> h_values = unittest::random_integers<unsigned int>(n);
    ...
    stable_radix_sort_by_key_kernel<<<1,1>>>(thrust::device, d_keys.begin(), d_keys.end(), d_values.begin(), is_supported.begin());
...
VariableUnitTest<TestRadixSortByKeyDeviceDevice, RadixSortKeyTypes> TestRadixSortByKeyDeviceDeviceInstance;
