...
void TestRadixSortSimpleDeviceDevice()
{
  ...
  stable_radix_sort_kernel<<<1,1>>>(thrust::device, unsorted_keys.begin(), unsorted_keys.end(), is_supported.begin());
...
DECLARE_UNITTEST(TestRadixSortSimpleDeviceDevice);
...
template<typename T>
struct TestRadixSortDeviceDevice
{
  void operator()(const size_t n)
  {
    thrust::host_vector<T>   h_keys = unittest::random_integers<T>(n);
    ...
    stable_radix_sort_kernel<<<1,1>>>(thrust::device, d_keys.begin(), d_keys.end(), is_supported.begin());
...
VariableUnitTest<TestRadixSortDeviceDevice, RadixSortKeyTypes> TestRadixSortDeviceDeviceInstance;
