  ...
  struct TestSortByKeyDeviceSeq
{
  void operator()(const size_t n)
  {
    ...
    sort_by_key_kernel<<<1,1>>>(thrust::seq, d_keys.begin(), d_keys.end(), d_values.begin(), thrust::less<T>(), is_supported.begin());
  ...
  struct TestSortByKeyDeviceDevice
{
  void operator()(const size_t n)
  {
    ...
    thrust::device_vector<bool> is_supported(1);
    sort_by_key_kernel<<<1,1>>>(thrust::device, d_keys.begin(), d_keys.end(), d_values.begin(), thrust::less<T>(), is_supported.begin());
