...
template<typename T>
struct TestRadixSortByKeyUnalignedDeviceDevice
{
  void operator()(const size_t n)
  {
    typedef thrust::device_vector<T>   Vector1;
    ...
    for(int offset = 1; offset < 4; offset++)
    {
        ...
        stable_radix_sort_by_key_kernel<<<1,1>>>(thrust::device, unaligned_unsorted_keys.begin() + offset, unaligned_unsorted_keys.end(), unaligned_unsorted_values.begin() + offset, is_supported.begin());
...
VariableUnitTest<TestRadixSortByKeyUnalignedDeviceDevice, UnsignedIntegerTypes> TestRadixSortByKeyUnalignedDeviceDeviceInstance;
