  ...
  struct TestSortDeviceSeq
{
  void operator()(const size_t n)
  {
    ...
    sort_kernel<<<1,1>>>(thrust::seq, d_data.begin(), d_data.end(), thrust::less<T>(), is_supported.begin());
  ...
  struct TestSortDeviceDevice
{
  void operator()(const size_t n)
  {
    ...
    thrust::device_vector<bool> is_supported(1);
    sort_kernel<<<1,1>>>(thrust::device, d_data.begin(), d_data.end(), thrust::less<T>(), is_supported.begin());
