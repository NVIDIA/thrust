...
typedef komradetest::type_list<int,
                               unsigned int,
                               float> ThirtyTwoBitTypes;

typedef komradetest::type_list<long long,
                               unsigned long long,
...
typedef komradetest::type_list<signed char,
                               signed short,
                               signed int,
                               signed long,
                               signed long long> SignedIntegralTypes;

typedef komradetest::type_list<unsigned char,
                               unsigned short,
                               unsigned int,
                               unsigned long,
...
void VTEST##Host(void)   {  VTEST< komrade::host_vector<short> >();   VTEST< komrade::host_vector<int> >();   }    \
void VTEST##Device(void) {  VTEST< komrade::device_vector<short> >(); VTEST< komrade::device_vector<int> >(); }    \
  ...
  class VariableUnitTest : public UnitTest
{
  public:
    VariableUnitTest()
      : UnitTest(base_class_name(komradetest::type_name<TestName<int> >()).c_str()) {}
    ...
    void run()
    {
        ...
        {                                                 
            ...
            typedef typename komradetest::get_type<TypeList,0>::type first_type;

            komradetest::for_each_type<TypeList,TestName,first_type,0> loop;
    ...
    : public UnitTest
{
  VectorUnitTest()
    : UnitTest("dummy_name")
  {
    std::string test_name = base_class_name(komradetest::type_name<TestName< Vector<int, Alloc<int> > > >());
    std::string vector_name = base_class_name(komradetest::type_name<Vector<int, Alloc<int> > >());
  ...
  void run()
  {
    ...
    typedef typename komradetest::transform1<TypeList, Alloc>::type AllocList;

    // zip up the type list & alloc list with Vector
    typedef typename komradetest::transform2<TypeList, AllocList, Vector>::type VectorList;

    // get the first type in the list
    typedef typename komradetest::get_type<VectorList,0>::type first_type;

    komradetest::for_each_type<VectorList,TestName,first_type,0> loop;
