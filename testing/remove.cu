...
void TestRemoveIfStencil(const size_t n)
{
    ...
    typename thrust::host_vector<T>::iterator   h_new_end = thrust::remove_if(h_data.begin(), h_data.end(), h_stencil.begin(), pred);
    typename thrust::device_vector<T>::iterator d_new_end = thrust::remove_if(d_data.begin(), d_data.end(), d_stencil.begin(), pred);
...
void TestRemoveCopyIfStencil(const size_t n)
{
    ...
    typename thrust::host_vector<T>::iterator   h_new_end = thrust::remove_copy_if(h_data.begin(), h_data.end(), h_stencil.begin(), h_output.begin(), pred);
    typename thrust::device_vector<T>::iterator d_new_end = thrust::remove_copy_if(d_data.begin(), d_data.end(), d_stencil.begin(), d_output.begin(), pred);

    h_data.resize(h_new_end - h_output.begin());
    d_data.resize(d_new_end - d_output.begin());
